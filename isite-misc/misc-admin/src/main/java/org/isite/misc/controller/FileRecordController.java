...
public class FileRecordController extends BaseController {
    ...
    public Result<Integer> updateFileRecord(@Validated(Update.class) @RequestBody FileRecordDto fileRecordDto) {
        FileRecordPo fileRecordPo = convert(fileRecordDto, FileRecordPo::new);
        return toResult(fileRecordService.updateSelectiveById(fileRecordPo));
    }

    @PostMapping(MY_POST_FILE_RECORD)
    public Result<Integer> addFileRecord(@Validated(Add.class) @RequestBody FileRecordDto fileRecordDto) {
        FileRecordPo fileRecordPo = toFileRecordPo(fileRecordDto, getUserId());
